 ...
 */
extern  struct tappoint_dev g_tap_dev;
...
ieee80211_rx_h_data(struct ieee80211_rx_data *rx)
{
	...
	/*
	*here we intercept the data packet,and put it into the rx_queue
	*/
	//err = __ieee80211_data_to_8023(rx, &port_control);
	rx->skb->dev=sdata->dev;
	/*down(&g_tap_dev.sem_guard);*/
	spin_lock_bh(&g_tap_dev.lock_guard);
	rc=enqueue_skb(&g_tap_dev,1,rx->skb);
	/*up(&g_tap_dev.sem_guard);*/
	spin_unlock_bh(&g_tap_dev.lock_guard);
   // printk("enqueue rx:%s\n",!(rc<0)?"succeed":"fails");
	
	if(rc<0)/*enqueue ops fails due to limited buffer and slow dequeue-ops,drop it*/
		goto drop_proc;
	wake_up_interruptible(&g_tap_dev.skb_wait);
	#if 0
	err = __ieee80211_data_to_8023(rx, &port_control);
	if (unlikely(err))
		return RX_DROP_UNUSABLE;
